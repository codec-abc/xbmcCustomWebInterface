...
NowPlayingManager.prototype = {
  ...
  bindPlaybackControls: function() {
	...
	//$('#slider').bind('slide',jQuery.proxy(this.debugHelp, $('#slider')));
    $('#slider').bind('slideStop',jQuery.proxy(this.debugHelp, this));
  },
  ...
  debugHelp : function()
  {
	var percent = $('#slider').data('slider').getValue();
console.log('test');
  ...
  refreshAudioData: function() {
    ...
    if (this.activePlaylistItem) {
      ...
      if (progressBarWidth <= 100) {
		...
		$('#slider').slider('setValue',progressBarWidth);
      }
    }
  },
